...
if ( !class_exists("Oworganic_Woo_Swatches") ) {
	class Oworganic_Woo_Swatches {

		public static function swatches_list( $image_size = 'woocommerce_thumbnail' ) {

			$show_swatches = oworganic_get_config( 'show_product_swatches_on_grid' );
			if ( !$show_swatches || !oworganic_is_woo_swatches_activated() ) {
				return;
			}
			...
			$attribute_name = oworganic_get_config('product_swatches_attribute');
				...
				if ( !empty($data) ) {
					...
					$output .= '<a href="javascript:void(0)" class="'.esc_attr($data['swatches_type']).'" data-image_src="'.esc_attr($data['image_src']).'" data-image_srcset="'.esc_attr($data['image_srcset']).'" data-image_sizes="'.esc_attr($data['image_sizes']).'">';
						switch ( $data['swatches_type'] ) {
							case 'color':
								$output .= sprintf( '<div class="swatch-preview swatch-color" style="background-color:%s" data-toggle="tooltip" title="%s"></div>', esc_attr( $data['swatches_value'] ), esc_attr( $data['title'] ) );
								break;
			...
			echo apply_filters( 'oworganic_swatches_list', $output, $product_id, $product );
		}
