...
if ( !class_exists("Oworganic_Woo_Brand") ) {
	class Oworganic_Woo_Brand {

		public static function init() {
			// brand
			$tax = oworganic_get_config( 'product_brand_attribute' );
			if ( !empty($tax) ) {
		...
		public static function add_brand() {
			...
			<div class="form-field">
				<label><?php esc_html_e( 'Thumbnail', 'oworganic' ); ?></label>
		...
		public static function edit_brand( $term ) {
			...
			?>
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'oworganic' ); ?></label></th>
				<td>
		...
		public static function brand_image_field( $image = '' ) {
			...
			<div class="upload_image_action">
	            <input type="button" class="button add-image" value="<?php esc_html_e( 'Add', 'oworganic' ); ?>">
	            <input type="button" class="button remove-image" value="<?php esc_html_e( 'Remove', 'oworganic' ); ?>">
	        </div>
			<?php
		}
		...
		public static function save_brand_image( $term_id ) {
			if ( isset($_POST['product_brand_image']) ) {
				update_woocommerce_term_meta( $term_id, 'product_brand_image', sanitize_text_field($_POST['product_brand_image']) );
			}
		...
		public static function brand_columns( $columns ) {
			...
			foreach ($columns as $key => $value) {
				if ( $key == 'name' ) {
					$new_columns['image'] = esc_html__( 'Image', 'oworganic' );
				}
		...
		public static function brand_column( $columns, $column, $id ) {
			if ( $column == 'image' ) {
				...
				$columns .= '<img src="' . esc_url( $image ) . '" alt="'.esc_attr__( 'Image', 'oworganic' ).'" class="wp-post-image" />';
			}
		...
		public static function get_product_brands() {
		    global $product;
		    $brands_tax = oworganic_get_config( 'product_brand_attribute' );
	...
	add_action( 'init', array('Oworganic_Woo_Brand', 'init') );
}