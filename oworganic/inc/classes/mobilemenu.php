...
if ( !class_exists("Oworganic_Mobile_Menu") ) {
    class Oworganic_Mobile_Menu extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'oworganic');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'oworganic');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'oworganic');
                        break;
        ...
        public function generateSubMegaMenu( $item, $apus_mega_profile ) {
            if ( $apus_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    $post = $posts[0];
                    $content = apply_filters( 'oworganic_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
                    $width  = $item->apus_width ? 'style="width:'.$item->apus_width.'px"':"";
                    return '<ul class="sub-menu" '.$width.'><li><div class="dropdown-menu-inner">'.$content.'</div></li></ul>';
                }
            }
            return '';
        }
