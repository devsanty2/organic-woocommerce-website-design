...
<body <?php body_class(); ?>>
<?php if ( oworganic_get_config('preload', true) ) {
	$preload_icon = oworganic_get_config('media-preload-icon');
	$styles = array();
...
<div id="wrapper-container" class="wrapper-container">
	...
	<?php
		$header = apply_filters( 'oworganic_get_header_layout', oworganic_get_config('header_type') );
		get_template_part( 'headers/'.$header );
		if ( !empty($header) ) {
			oworganic_display_header_builder($header);
		} else {
